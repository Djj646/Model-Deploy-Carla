...
parser.add_argument('--dataset_name', type=str, default="cgan-human-data-09-clip-0.1", help='name of the dataset')
...
train_loader = DataLoader(CARLADataset(data_index=[7]),
                            batch_size=opt.batch_size, shuffle=False, num_workers=opt.n_cpu)
test_val_dataloader = DataLoader(CARLADataset(data_index=[1], eval_mode=True),
...
for epoch in range(opt.epoch, opt.n_epochs):
    ...
    for i, batch in bar:
        ...
        #loss_G = loss_pixel
        loss_G.backward()
        torch.nn.utils.clip_grad_value_(generator.parameters(), clip_value=0.1)
        optimizer_G.step()
        ...
        loss_D.backward()
        torch.nn.utils.clip_grad_value_(discriminator.parameters(), clip_value=0.1)
        optimizer_D.step()
