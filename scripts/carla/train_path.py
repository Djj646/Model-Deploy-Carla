...
from learning.model_train_path import Traj
...
parser.add_argument('--dataset_name', type=str, default="train-03", help='name of the dataset')
...
# model = ModelGRU(256).to(device) # wyk 's
model = Traj().to(device) # whj 's

train_loader = DataLoader(CostMapDataset(data_index=[7], opt=opt, dataset_path='/data2/wanghejun/DATASET/CARLA/town01/'), batch_size=opt.batch_size, shuffle=False, num_workers=opt.n_cpu)

test_loader = DataLoader(CostMapDataset(data_index=[1], opt=opt, dataset_path='/data2/wanghejun/DATASET/CARLA/town01/', evalmode=True), batch_size=1, shuffle=False, num_workers=1)
