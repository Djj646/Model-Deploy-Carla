...
# 输入RGB相片，本地导航路线图，当前时间，速度
# 输出连续轨迹y(t)
class TrajFrom2Pic(nn.Module):
    """
    Total Network from Simple Input to Continuous Trajectory
    Input(RGB image V, Local Route R,t,v0)-->CNN-->RNN-->merge-->MLP-->Output(x,y)
    ----------------------------------------------------------------------------
    Input:
    ...
    def __init__(self, mid_dim: int = 256):
        ...
        self.cnn1 = CNN(in_dim=3, out_dim=mid_dim)
        # in_dim = 3 for RGB image
        self.gru1 = GRU(hidden_dim=mid_dim)

        self.cnn2 = CNN(in_dim=3, out_dim=mid_dim)
        self.gru2 = GRU(hidden_dim=mid_dim)

        self.merge = nn.Linear(in_features=512, out_features=256)
        self.mlp = MLP(input_dim=258, rate=1.0, output=2)

    @staticmethod
    def img_process(x, cnn, rnn):
        """
        Network contain CNN and RNN that V and R go through Separately
        :param x: 5D tensor:batch_size*number_per_stack*color*height*width
        :param cnn:CNN to use
        :param rnn:RNN to use
        ...
        x = cnn(x)  # 4D tensor:batch_size_time_steps*CNN_output_Depth*CNN_output_Height*CNN_output_Width
        x = x.view(batch_size, time_steps, -1)  # 4D --> 3D,batch_size*time_steps*Length
        x, _, = rnn(x)  # Dimension No Change,But 2nd dimension becomes "result of the x time"
        x = functional.leaky_relu(x[:, -1, :])  # 3D --> 2D,Dimension*Length
        return x

    def forward(self, image, routine, t, v0):
        # using cnn1, gru1, cnn2, gru2 neural network
        x1 = self.img_process(image, self.cnn1, self.gru1)
        x2 = self.img_process(routine, self.cnn2, self.gru2)
        
        # 5D --> 2D
        # batch_size*number_per_stack*color*height*width --> batch_size*Length
...
if __name__ == '__main__':
    a = torch.arange(24.0).reshape(2, 3, 4)
    print(a.shape)
    # v0 = torch.Tensor([[2], [2]])
